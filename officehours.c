...
static int student_request(int class)
{
   ...
   if(permit == 1)
   {
        student_permitted_yet++;
        if(student_permitted_yet >5)
        {
            permit = 0;
        }
   }
   ...
   {    
        ...
        if(value == 0){pthread_mutex_unlock(&mutex); return 0;}
        printf("The value of semaphore is %d.\n", value);
        sem_wait(&office_chairs);
        printf("going ahead of semaphore wait\n");
        printf("Permission granted to the student from class %d\n", class);
...
/*time for break so stop alla students*/
...
static void classa_leave() 
{
  ...
  if(students_since_break >=5)
  {
     printf("\nStudents since break is %d. Time to take break\n", students_since_break);
     take_break();
     students_since_break = 0;
     student_permitted_yet = 0; 
  }
 
  printf("Class A student leaves and the number of students in office is %d\n", classa_inoffice);
  pthread_mutex_unlock(&mutex);
}
...
static void classb_leave() 
{
  ...
  if(students_since_break >=5)
  {
     printf("\nProfessor need to take break\n");
     take_break();
     students_since_break = 0;
     student_permitted_yet = 0; 
  }
 
  printf("Class B student leaves and the number of students in office is %d\n", classb_inoffice);
  pthread_mutex_unlock(&mutex);  
}
